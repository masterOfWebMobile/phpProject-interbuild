...
$query = "update folders set folder_deleted = 0 where folder_id={$_POST['did']}";
mysqli_query($cxn, $query);
$query = "update documents set document_deleted = 0 where folderid={$_POST['did']}";
mysqli_query($cxn, $query);

$uquery = "Select * from accounts where account_userid={$_SESSION['userid']}";
$ures = mysqli_query($cxn, $uquery);
$urow = mysqli_fetch_assoc($ures);
$detail = $urow['account_firstname']." ".$urow['account_lastname']." has recovered ".$row['folder_name']." folder";
